...
const Services = () => {
  ...
  // Preload all service images on component mount
  useEffect(() => {
      ...
      try {
        const imagePromises = services.map(service => {
          return new Promise((resolve, reject) => {
            const img = new Image();
            img.onload = () => resolve(service.img);
            img.onerror = reject;
            img.src = service.img;
          });
        });
        ...
        console.log('✅ Services images cached - no more network requests!');
      } catch (error) {
        console.error('Error preloading Services images:', error);
